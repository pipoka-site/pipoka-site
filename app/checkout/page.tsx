...
export default function CheckoutPage() {
  ...
  const pickupAddressLabel = settings.pickup_address?.trim() || "";
  const pickupQuery = pickupAddressLabel ? encodeURIComponent(pickupAddressLabel) : "";
  const pickupGoogleMapsUrl = (settings.pickup_google_maps_url || "").trim() || (pickupQuery ? `https://www.google.com/maps/search/?api=1&query=${pickupQuery}` : "");
  const pickupWazeUrl = (settings.pickup_waze_url || "").trim() || (pickupQuery ? `https://waze.com/ul?q=${pickupQuery}&navigate=yes` : "");
  const pickupAppleMapsUrl = (settings.pickup_apple_maps_url || "").trim() || (pickupQuery ? `https://maps.apple.com/?q=${pickupQuery}` : "");
    ...
    <section className="container-pipoka px-4 py-10 pb-[calc(1.5rem+env(safe-area-inset-bottom))] sm:px-6 md:py-16">
      <div className="mx-auto grid max-w-6xl gap-8 lg:grid-cols-[1.05fr_.95fr]">
        ...
        {sessionStatus === "guest" ? <div className="h-fit rounded-[2rem] bg-white p-6 text-center shadow-soft md:p-8"><h2 className="font-serif text-3xl font-bold text-wine-900">Entre para finalizar</h2><p className="mt-3 text-wine-900/65">Seu carrinho está salvo. Entre ou crie uma conta para usar seus dados e endereços cadastrados.</p><Link href="/conta?next=/checkout" className="btn-primary mt-6 w-full">Entrar ou criar conta</Link></div> : loadingAccount || sessionStatus === "checking" ? <div className="h-fit rounded-[2rem] bg-white p-8 text-center shadow-soft">Carregando seus dados...</div> : <form onSubmit={submit} className="w-full rounded-[2rem] bg-white p-5 shadow-soft sm:p-6 md:p-8">
          ...
          {delivery === "Retirada" && <div className="mt-4 overflow-hidden rounded-2xl bg-cream text-sm text-wine-900/75"><div className="p-4"><div className="flex gap-3"><MapPin className="mt-0.5 shrink-0 text-gold-600" size={19}/><div>{settings.pickup_address && <strong className="block text-wine-900">{settings.pickup_address}</strong>}<p>{settings.pickup_instructions}</p>{(pickupGoogleMapsUrl || pickupWazeUrl || pickupAppleMapsUrl) && <div className="mt-3 flex flex-wrap gap-2">{pickupGoogleMapsUrl && <a href={pickupGoogleMapsUrl} target="_blank" rel="noreferrer" className="rounded-full border border-wine-900/20 bg-white px-3 py-1.5 text-xs font-semibold text-wine-800">Google Maps</a>}{pickupWazeUrl && <a href={pickupWazeUrl} target="_blank" rel="noreferrer" className="rounded-full border border-wine-900/20 bg-white px-3 py-1.5 text-xs font-semibold text-wine-800">Waze</a>}{pickupAppleMapsUrl && <a href={pickupAppleMapsUrl} target="_blank" rel="noreferrer" className="rounded-full border border-wine-900/20 bg-white px-3 py-1.5 text-xs font-semibold text-wine-800">Apple Maps</a>}</div>}</div></div></div></div>}
